...
protected:
 ...
 //  void              EventSLToL(bool,bool,string);
   ...
   string            fNameOrderType(int);// long to int
...
double CIntegerTradePanel::fLotsNormalize(double aLots)
  {
   ...
   if(aLots> SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN) ){
 ...
 //  aLots+=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN);
   ...
   // ensure lot size is greater than the min and the max 
...
void CIntegerTradePanel::FormCreate()
  {
   ...
   PosX=ChartWidth-Sept2018-OffsetX;
   PosY=ChartHeigh-40-OffsetY;
   // size of the form 
   // 327 original
   // size of another button 
   // 377 
   
   //392 too big
   fObjCreateEdit(NameForm,"",PosX,PosY,Sept2018,37,0,ANCHOR_LEFT_UPPER,CORNER_LEFT_UPPER,ColorFormBg,ColorFormBorder,7,"Arial",0,false,0,OBJ_ALL_PERIODS,true,false);
   FormSolvePos();
   FormFrameSetPos();
   FormControlsCreate();
   ChartRedraw(0);
...
void CIntegerTradePanel::FormBuySellButtonsCreate()
  {
   int TmpSellLeft=PosX+165;//195 + 26
   int TmpBuyLeft=PosX+246;//246
   ...
   //new 
   int TmpDeleteLeft=PosX+327;  
   fObjCreateButton(NameDeleteBut,false,TmpDeleteLeft+1,PosY+3,50,31,"Del",ColorTradeBut[2],ColorText,0,CORNER_LEFT_UPPER,10,"Arial",0,false,OBJ_ALL_PERIODS);
...
void CIntegerTradePanel::EventSLToL(bool aSoundON,bool aEMailON,string aSoundFile)
  ...
  */
//+------------------------------------------------------------------+
//|    Deal Profit minus the commmission                              |
//+------------------------------------------------------------------+
...
//|                                                                  |
//+------------------------------------------------------------------+
string CIntegerTradePanel::fNameOrderType(int aType)
  {
   switch(aType)
     {
   ...
   if(id==CHARTEVENT_OBJECT_CLICK)
     {
      ...
      if(sparam==NameSellBut)
        ...
        // ADD DELETE THE ORDERS 
         if(sparam==NameDeleteBut)
        {
        
         fOpDelete(_Symbol,Magic);
            ...
            if(ObjectGetInteger(0,NameSelect[i],OBJPROP_STATE))
              {
               ...
               // DELETE THE BUY AND SELL BUTTONS 
                  // THE USER HAS SELECTED ANOTHER BUTTON 
               FormBuySellButtonDelete();
               
               // CREATE NEW BUTTON BASED ON THE 
      ...
      if(sparam==NameStopPriceLine || sparam==NameOpenPriceLine || sparam==NameSLPriceLine || sparam==NameTPPriceLine || sparam==NameExpirTimeLine)
        {
         ...
         datetime Expiration;
         /*
         // Get the values of the Line Objects 
        */
...
//+------------------------------------------------------------------+
//|       Modify the Values for the Lines to be drawn                |
//+------------------------------------------------------------------+

input double buttonsPendingOrderDistanceMultiplier = 2;
...
void CIntegerTradePanel::LinesSLTPSolveAndCreate()
  {
   ...
   // get spread from the chart symbol 
   double spr=(int)SymbolInfoInteger(_Symbol,SYMBOL_SPREAD);
   
   

   DefaultLineTriggerLevel=(spr*7)*buttonsPendingOrderDistanceMultiplier;
   DefaultLinePendingLevel=(spr*14)* buttonsPendingOrderDistanceMultiplier;
   DefaultLineStopLoss=(spr*14)* buttonsPendingOrderDistanceMultiplier;
   DefaultLineTakeProfit=(spr*28)* buttonsPendingOrderDistanceMultiplier;
...
//|                                                                  |
//+------------------------------------------------------------------+ 
bool CIntegerTradePanel::fOpDelete(string Sym ,ulong getmagic)
{
   User_Request = true;
...
//--- iterate over all placed pending orders
   ...
   for(int i=total-1; i>=0; i--)
    ...
    string order_sym = OrderGetString(ORDER_SYMBOL);
      //--- if the MagicNumber matches
      if(magic==getmagic && order_sym == Sym)
        {
...
//+------------------------------------------------------------------+  
  ...
  input ENUM_ORDER_TYPE_FILLING Broker_Order_Filling = ORDER_FILLING_IOC;
  
  input string ENDOFTASK ="------------------------------------------------------------------------------------------";
...
//ie=false;
   ...
   //ORDER_FILLING_FOK;
   request.type_filling=Broker_Order_Filling;
