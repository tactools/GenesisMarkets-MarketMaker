...
public:
   ...
   //+------------------------------------------------------------------+
   void  SetText(const int row,const string text,const color colour, const string market)
  // void  SetText(int row,string text,color colour)
     {
      if(row<0 || market != Symbol() )
         return;
      //---
      ...
      if(row<total)
        {
         ...
         item.text=text;
         item.colour=colour;
       //  Print(" Row " + row + " < Total in List " +  total  );
