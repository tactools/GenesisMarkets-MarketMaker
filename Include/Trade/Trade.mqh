...
/*
October 4, 2018
Make change for EXCHANGE execution
...
//--- check filling
   if(!FillingCheck(symbol)){
 if(TRADE_DEBUG)  Print(" ERROR in FillingCheck, returning FALSE");
      return(false);   
   } 
//--- check order type
   if(order_type==ORDER_TYPE_BUY || order_type==ORDER_TYPE_SELL)
     {
      m_result.retcode=TRADE_RETCODE_INVALID;
      m_result.comment="Invalid order type";
    if(TRADE_DEBUG)  Print("returning FALSE, not the right order Type ");
      return(false);
     }
//--- check order expiration
      ...
      if((exp&SYMBOL_EXPIRATION_GTC)!=SYMBOL_EXPIRATION_GTC)
        {
         ...
         if(TRADE_DEBUG)   Print(__FUNCTION__,": Error: Unable to place order without explicitly specified expiration time");
  ...
  if(TRADE_DEBUG)   Print("WHAT ORDER FILLING TYPE IS PRESENT " + IntegerToString(m_type_filling) );
//--- setting request
...
//--- check order type
   if(!OrderTypeCheck(symbol)){
    if(TRADE_DEBUG)  Print("OrderTypeCheck() , returning False " );
      return(false);
   }
//--- check filling
   if(!FillingCheck(symbol))
    ...
    if(TRADE_DEBUG)  Print(__FUNCTION__+": Invalid filling type" + IntegerToString(m_type_filling) );
      return(false);
     }
//--- check expiration
   if(!ExpirationCheck(symbol))
     {
      m_result.retcode=TRADE_RETCODE_INVALID_EXPIRATION;
   if(TRADE_DEBUG)   Print(__FUNCTION__+": Invalid expiration type");
      return(false);
     }
   m_request.comment=comment;
//--- action and return the result
...
//---
   string str;
 if(TRADE_DEBUG)  PrintFormat("%s",FormatRequest(str,m_request));
  }
//+------------------------------------------------------------------+
...
//---
   string str;
 if(TRADE_DEBUG)  PrintFormat("%s",FormatRequestResult(str,m_request,m_result));
  }
//+------------------------------------------------------------------+
...
//--- get possible filling policy types by symbol
   ...
   // what is the value
 if(TRADE_DEBUG)  Print(" Filling Type for the Symbol " + symbol + " value = " + IntegerToString(filling) );
   
   if((filling&SYMBOL_FILLING_FOK)==SYMBOL_FILLING_FOK)
     {
    if(TRADE_DEBUG)  Print(" Filling Type (Selected) for the Symbol " + symbol + " FOK " );
      m_type_filling=ORDER_FILLING_FOK;
      return(true);
     }
   if((filling&SYMBOL_FILLING_IOC)==SYMBOL_FILLING_IOC)
     {
   if(TRADE_DEBUG)  Print(" Filling Type (Selected) for the Symbol " + symbol + " IOC " );
     ...
     if(filling==2)
     {
    if(TRADE_DEBUG)  Print(" Filling Type (Selected) for the Symbol " + symbol + " RETURN " );
...
bool CTrade::FillingCheck(const string symbol)
  {
  if(TRADE_DEBUG)  Print("DEBUG: FillingCheck() " + symbol );
...
//--- check execution mode
   if(exec==SYMBOL_TRADE_EXECUTION_REQUEST || exec==SYMBOL_TRADE_EXECUTION_INSTANT)
    ...
    if(TRADE_DEBUG)  Print(" FillingCheck() == SYMBOL_TRADE_EXECUTION_REQUEST  || SYMBOL_TRADE_EXECUTION_INSTANT " );
      return(true);
     }
//--- get possible filling policy types by symbol
   uint filling=(uint)SymbolInfoInteger(symbol,SYMBOL_FILLING_MODE);
    // what is the value
  if(TRADE_DEBUG)   Print("FillingCheck() Filling Type for the Symbol " + symbol + " value = " + IntegerToString(filling) );
         ...
         if((filling&SYMBOL_FILLING_FOK)==SYMBOL_FILLING_FOK)
           {
            m_type_filling=ORDER_FILLING_FOK;
            m_request.type_filling=m_type_filling;
       if(TRADE_DEBUG)     Print("FillingCheck() Filling Type (Selected) for the Symbol " + symbol + " FOK " );
            return(true);
           }
         if((filling&SYMBOL_FILLING_IOC)==SYMBOL_FILLING_IOC)
           {
            m_type_filling=ORDER_FILLING_IOC;
            m_request.type_filling=m_type_filling;
        if(TRADE_DEBUG)    Print("FillingCheck() Filling Type (Selected) for the Symbol " + symbol + " IOC " );
            return(true);
           }
         //--- wrong filling policy, set error code
         m_result.retcode=TRADE_RETCODE_INVALID_FILL;
      if(TRADE_DEBUG)   Print("FillingCheck() Filling Type (Selected) for the Symbol " + symbol + " WRONG VALUE NEED TO FIX THIS " );
         return(false);
        }
    if(TRADE_DEBUG)  Print("FillingCheck() returning TRUE " );
      return(true);
     }
 if(TRADE_DEBUG)   Print("FillingCheck() is not SYMBOL_TRADE_EXECUTION_MARKET , proceeding... " );  
...
//--- EXCHANGE execution mode
   ...
   m_type_filling=ORDER_FILLING_RETURN;
 if(TRADE_DEBUG)  Print("FillingCheck() , changing m_type_filling to ORDER_FILLING_RETURN , value = " + IntegerToString(m_type_filling) ); 
   ...
   switch(m_type_filling)
     {
      case ORDER_FILLING_FOK:
         ...
         if(m_request.action==TRADE_ACTION_PENDING)
           ...
           if(TRADE_DEBUG)    Print("EXCHANGE Filling Type (Selected) for the Symbol " + symbol + " ORDER_FILLING_RETURN " );
               return(true);
              }
           }
         ...
         if((filling&SYMBOL_FILLING_FOK)==SYMBOL_FILLING_FOK)
           {
            m_request.type_filling=m_type_filling;
       if(TRADE_DEBUG)     Print("EXCHANGE Filling Type (Selected) for the Symbol " + symbol + " FOK " );
            return(true);
           }
         //--- wrong filling policy, set error code
         m_result.retcode=TRADE_RETCODE_INVALID_FILL;
    if(TRADE_DEBUG)     Print("EXCHANGE Filling Type returning False for ORDER_FILLING_FOK " + symbol + " m_type_filling == " + IntegerToString(m_type_filling) );
         return(false);
         ...
         if(m_request.action==TRADE_ACTION_PENDING)
           {
            ...
            if(m_request.type==ORDER_TYPE_BUY_STOP || m_request.type==ORDER_TYPE_SELL_STOP ||
               m_request.type==ORDER_TYPE_BUY_LIMIT || m_request.type==ORDER_TYPE_SELL_LIMIT)
              {
               ...
               //--- add the corresponding filling policy to the request
               m_request.type_filling=ORDER_FILLING_RETURN;
       if(TRADE_DEBUG)        Print("EXCHANGE Filling Type (Selected) for the Symbol " + symbol + " return ORDER_FILLING_RETURN " );
               return(true);
              }
           }
         //--- in case of limit order or instant execution order
         ...
         if((filling&SYMBOL_FILLING_IOC)==SYMBOL_FILLING_IOC)
           {
            m_request.type_filling=m_type_filling;
        if(TRADE_DEBUG)    Print("EXCHANGE Filling Type (Selected) for the Symbol " + symbol + " IOC " );
            return(true);
           }
         //--- wrong filling policy, set error code
         m_result.retcode=TRADE_RETCODE_INVALID_FILL;
      if(TRADE_DEBUG)   Print("EXCHANGE Filling Type (Selected) for the Symbol " + symbol + " INVALID NEED TO FIX " );
         return(false);
...
//--- unknown execution mode, set error code
 ...
 if(TRADE_DEBUG)  Print("EXCHANGE Filling Type (Selected) for the Symbol " + symbol + " ERROR " );
   return(false);
  }
//+------------------------------------------------------------------+
   ...
   switch(m_request.type_time)
     {
      ...
      case ORDER_TIME_SPECIFIED_DAY:
         ...
         break;
      default:
      if(TRADE_DEBUG)   Print(__FUNCTION__+": Unknown expiration type");
         break;
     }
//--- failed
   return(false);
...
//--- depending on the type of order in request
   ...
   if(TRADE_DEBUG)   Print(__FUNCTION__+": Invalid order type");
     }
//--- result
   return(res);
  }
   ...
   if(res)
     {
      ...
      if(TRADE_DEBUG)   PrintFormat(__FUNCTION__+": %s [%s]",FormatRequest(action,request),FormatRequestResult(fmt,request,result));
     }
   else
     {
